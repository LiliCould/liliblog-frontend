  ...
  <aside class="hidden lg:flex flex-col gap-6 sticky top-24 self-start">
    ...
    <div
      ...
      <div v-if="loading" class="flex flex-col gap-3">
        <div v-for="i in 5" :key="i" class="h-4 bg-[rgba(var(--color-primary-rgb),0.06)] rounded animate-pulse"></div>
      </div>
    ...
    <div
      ...
      <div
        class="flex flex-wrap gap-2.5 max-h-[26rem] overflow-y-auto p-1 [&::-webkit-scrollbar]:w-[5px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-[rgba(var(--color-primary-rgb),0.15)] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb:hover]:bg-[rgba(var(--color-primary-rgb),0.3)]">
        <router-link v-for="tag in appStore.tags" :key="tag.id" :to="`/tag/${tag.name}`"
          class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold no-underline border border-[rgba(var(--color-primary-rgb),0.15)] bg-[rgba(var(--color-primary-rgb),0.06)] transition-all duration-200 hover:-translate-y-px hover:[box-shadow:0_0_8px_rgba(var(--color-primary-rgb),0.15)] hover:border-[rgba(var(--color-primary-rgb),0.4)]"
          :style="{ color: tag.color || '#00f0ff', backgroundColor: tag.color ? tag.color + '15' : 'rgba(0,240,255,0.06)' }">
          <span class="w-1.5 h-1.5 rounded-full opacity-70 [box-shadow:0_0_6px_currentColor]"
            :style="{ backgroundColor: tag.color || '#00f0ff' }"></span>
          {{ tag.name }}
        </router-link>
