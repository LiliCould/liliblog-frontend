<template>
  <header class="fixed top-0 left-0 right-0 h-16 z-50 transition-all duration-300"
    :class="isScrolled ? 'bg-[rgba(10,10,15,0.95)] shadow-[0_2px_20px_rgba(var(--color-primary-rgb),0.08)]' : 'bg-[rgba(10,10,15,0.85)]'"
    ...
    <div class="h-full max-w-7xl mx-auto px-4 sm:px-6 flex items-center">
      ...
      <nav class="hidden md:flex items-center gap-1.5 ml-6 shrink-0">
        <router-link v-for="item in navItems" :key="item.path" :to="item.path"
          class="relative flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium text-cyber-body no-underline rounded-md transition-all duration-200 hover:text-cyber-primary hover:bg-[rgba(var(--color-primary-rgb),0.08)]"
          active-class="!text-cyber-primary !font-semibold [text-shadow:0_0_8px_rgba(var(--color-primary-rgb),0.4)]">
          <component :is="item.icon" class="w-4 h-4" />
          <span>{{ item.label }}</span>
          <span
            class="absolute bottom-1 left-1/2 -translate-x-1/2 w-3/5 h-0.5 bg-cyber-primary rounded-full scale-x-0 transition-transform duration-300 [box-shadow:0_0_6px_rgba(var(--color-primary-rgb),0.5)]"
        ...
        <div class="hidden md:flex w-full max-w-[400px]">
          <div class="flex-1 relative">
            ...
            <input v-model="searchKeyword" type="text" placeholder="搜索文章..."
              class="w-full h-9 pl-9 pr-4 rounded-full bg-[rgba(17,17,24,0.6)] border border-[rgba(var(--color-primary-rgb),0.12)] text-white placeholder-[#6b7280] text-sm outline-none transition-all duration-300 focus:border-[rgba(var(--color-primary-rgb),0.4)] focus:shadow-[0_0_12px_rgba(var(--color-primary-rgb),0.1)]"
      ...
      <div class="flex items-center gap-2 shrink-0">
          ...
          <div class="relative" ref="dropdownRef">
            ...
            <Transition enter-active-class="transition duration-200 ease-out"
              ...
              <div v-if="showDropdown"
                class="absolute right-0 top-full mt-2 w-52 bg-cyber-surface border border-t-border rounded-lg overflow-hidden [box-shadow:0_8px_32px_rgba(0,0,0,0.4),0_0_1px_rgba(var(--color-primary-rgb),0.3)]">
                ...
                <button
                  class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-cyber-body hover:text-cyber-primary hover:bg-[rgba(var(--color-primary-rgb),0.06)] transition-colors duration-200 cursor-pointer"
                ...
                <button
                  class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-cyber-pink hover:bg-[rgba(var(--color-secondary-rgb),0.06)] transition-colors duration-200 cursor-pointer"
        ...
        <template v-else>
          <router-link to="/login"
            class="hidden md:inline-flex px-4 py-2 text-sm font-medium text-cyber-primary border-[1.5px] border-[rgba(var(--color-primary-rgb),0.4)] rounded-md no-underline transition-all duration-200 hover:text-white hover:border-cyber-primary hover:bg-[rgba(var(--color-primary-rgb),0.1)] hover:-translate-y-px [box-shadow:0_0_8px_rgba(var(--color-primary-rgb),0.15)]">
            登录
          </router-link>
          <router-link to="/register"
            class="hidden md:inline-flex px-4 py-2 text-sm font-medium text-white bg-cyber-primary rounded-md no-underline transition-all duration-200 hover:-translate-y-0.5 [box-shadow:0_4px_12px_rgba(var(--color-primary-rgb),0.25)] hover:[box-shadow:0_6px_20px_rgba(var(--color-primary-rgb),0.35),0_0_12px_rgba(var(--color-primary-rgb),0.2)]">
            注册
          </router-link>
        </template>

        <button
          class="md:hidden flex items-center justify-center w-10 h-10 text-cyber-title rounded-md transition-all duration-200 hover:bg-[rgba(var(--color-primary-rgb),0.08)] hover:text-cyber-primary cursor-pointer"
