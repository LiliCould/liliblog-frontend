<template>
  <div class="min-h-screen bg-cyber-bg flex">
    <aside
      class="hidden md:flex flex-col w-60 bg-cyber-surface border-r border-t-border flex-shrink-0 sticky top-0 h-screen">
      <div class="px-5 py-6 border-b border-t-border">
        <router-link to="/" class="flex items-center gap-2.5 no-underline group">
          <div
            class="w-8 h-8 rounded-sm bg-cyber-primary flex items-center justify-center text-white font-bold text-sm [box-shadow:0_0_12px_rgba(var(--color-primary-rgb),0.3)]">
            L
          </div>
          <span
            class="text-lg font-bold bg-gradient-to-r from-cyber-primary to-cyber-pink bg-clip-text [-webkit-text-fill-color:transparent]">LiliBlog</span>
        </router-link>
      </div>
      ...
      <nav class="flex-1 py-4 overflow-y-auto">
        ...
        <router-link v-for="item in navItems" :key="item.path" :to="item.path"
          ...
          :class="isActive(item.path) ? 'text-cyber-primary bg-[rgba(var(--color-primary-rgb),0.1)] [box-shadow:inset_2px_0_0_#00f0ff]' : 'text-cyber-body hover:text-cyber-primary hover:bg-[rgba(var(--color-primary-rgb),0.06)]'">
      ...
      <div class="px-3 py-4 border-t border-t-border">
        <router-link to="/"
          class="flex items-center gap-3 mx-3 px-3 py-2.5 rounded-sm text-sm font-medium text-cyber-muted no-underline transition-all duration-200 hover:text-cyber-primary hover:bg-[rgba(var(--color-primary-rgb),0.06)]">
    ...
    <div class="flex-1 flex flex-col min-w-0">
      <header
        class="sticky top-0 z-40 h-14 bg-[rgba(10,10,15,0.9)] border-b border-t-border flex items-center justify-between px-6"
        style="backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);">
        <div class="flex items-center gap-3">
          <button
            class="md:hidden flex items-center justify-center w-9 h-9 text-cyber-body rounded-sm transition-colors duration-200 hover:text-cyber-primary hover:bg-[rgba(var(--color-primary-rgb),0.06)] cursor-pointer"
        ...
        <div class="flex items-center gap-3">
          <div v-if="userStore.isLoggedIn" class="flex items-center gap-2">
            <div
              class="w-8 h-8 rounded-full bg-cyber-primary flex items-center justify-center text-white text-xs font-semibold overflow-hidden">
    ...
    <Teleport to="body">
      ...
      <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="-translate-x-full"
        ...
        <div v-if="sidebarOpen"
          class="fixed top-0 left-0 bottom-0 w-60 z-[70] bg-cyber-surface border-r border-t-border flex flex-col md:hidden">
          <div class="px-5 py-6 border-b border-t-border flex items-center justify-between">
            <router-link to="/" class="flex items-center gap-2.5 no-underline" @click="sidebarOpen = false">
              <div
                class="w-8 h-8 rounded-sm bg-cyber-primary flex items-center justify-center text-white font-bold text-sm">
                L</div>
              <span
                class="text-lg font-bold bg-gradient-to-r from-cyber-primary to-cyber-pink bg-clip-text [-webkit-text-fill-color:transparent]">LiliBlog</span>
            </router-link>
            <button
              class="flex items-center justify-center w-8 h-8 text-cyber-muted cursor-pointer transition-colors duration-200 hover:text-cyber-primary"
