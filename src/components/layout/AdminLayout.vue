<template>
  <div class="min-h-screen bg-t-bg flex">
    ...
    <div class="flex-1 flex flex-col min-w-0">
      <header
        class="sticky top-0 z-40 h-14 bg-[rgba(var(--color-bg-rgb),0.9)] border-b border-t-border flex items-center justify-between px-3 md:px-6"
      ...
      <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="-translate-x-full"
        ...
        <div v-if="sidebarOpen"
          ...
          <nav class="flex-1 py-4 overflow-y-auto">
            ...
            <router-link v-for="item in navItems" :key="item.path" :to="item.path"
              class="flex items-center gap-3 mx-3 px-3 py-2.5 text-sm font-medium no-underline transition-all duration-200"
              :class="isActive(item.path) ? 'text-t-primary bg-[rgba(var(--color-primary-rgb),0.1)] [box-shadow:inset_2px_0_0_var(--color-primary)]' : 'text-t-body hover:text-t-primary hover:bg-[rgba(var(--color-primary-rgb),0.06)]'"
              @click="sidebarOpen = false">
              <component :is="item.icon" class="w-[18px] h-[18px]" />
              <span>{{ item.label }}</span>
...
import { LayoutDashboard, FileText, FolderOpen, Tag, MessageSquare, Users, ScrollText, ArrowLeft, Menu, X } from 'lucide-vue-next'
...
const navItems = [
  ...
  { path: '/admin/audit-logs', label: '审计日志', icon: ScrollText },
]
