  ...
  <aside class="hidden lg:flex flex-col gap-6 sticky top-24 self-start">
    <div
      ...
      <div class="flex items-center gap-2.5 mb-3 pb-2 border-b border-t-border">
        <FolderOpen
          class="w-5 h-5 text-cyber-primary [filter:drop-shadow(0_0_3px_rgba(var(--color-primary-rgb),0.3))]" />
      ...
      <div
        ...
        <router-link v-for="cat in appStore.categories" :key="cat.id" :to="`/category/${cat.slug}`"
          class="flex items-center justify-between px-3.5 py-2.5 rounded-md text-sm font-medium text-cyber-body no-underline transition-all duration-250 relative hover:text-cyber-primary hover:bg-[rgba(var(--color-primary-rgb),0.06)] hover:translate-x-1 group/cat">
          <span
            class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-0 bg-gradient-to-b from-cyber-primary to-cyber-pink rounded-full transition-[height] duration-250 [box-shadow:0_0_6px_rgba(var(--color-primary-rgb),0.4)] group-hover/cat:h-3/5"></span>
          <span>{{ cat.name }}</span>
          <ChevronRight
        ...
        <div v-if="appStore.categories.length === 0"
          class="flex flex-col items-center gap-2 py-8 text-cyber-muted text-sm">
