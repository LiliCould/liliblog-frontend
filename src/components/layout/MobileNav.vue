  ...
  <Teleport to="body">
    ...
    <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="translate-x-full"
      ...
      <div v-if="appStore.isMobileNavOpen"
        class="fixed top-0 right-0 bottom-0 w-[280px] z-[70] bg-[rgba(10,10,15,0.95)] border-l border-t-border flex flex-col"
        ...
        <div class="flex items-center justify-between px-5 py-4 border-b border-t-border relative">
          <span
            class="text-lg font-bold text-cyber-primary [text-shadow:0_0_8px_rgba(var(--color-primary-rgb),0.3)]">LiliBlog</span>
          <button
            class="flex items-center justify-center w-8 h-8 text-cyber-muted cursor-pointer transition-all duration-200 hover:text-cyber-primary [filter:hover:drop-shadow(0_0_4px_rgba(var(--color-primary-rgb),0.4))]"
            @click="appStore.closeMobileNav()">
            <X class="w-5 h-5" />
          </button>
          <div
            class="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-cyber-primary to-transparent opacity-40">
          </div>
        </div>
        ...
        <div class="flex-1 py-4 overflow-y-auto">
          <template v-if="userStore.isLoggedIn">
            <div class="flex items-center gap-3 px-5 py-3 mb-2">
              <div
                class="w-12 h-12 rounded-full bg-cyber-primary flex items-center justify-center text-white text-base font-semibold overflow-hidden">
                <img :src="resolveAvatar(userStore.avatar)" alt="" class="w-full h-full object-cover"
                  @error="handleAvatarError" />
              </div>
              <div class="flex-1 min-w-0">
                <div class="text-[15px] font-semibold text-cyber-title truncate">{{ userStore.nickname ||
          ...
          <nav class="flex flex-col">
            ...
            <template v-if="userStore.isLoggedIn">
              ...
              <a class="flex items-center gap-3 px-5 py-3 text-sm text-cyber-pink cursor-pointer transition-all duration-200 relative hover:bg-[rgba(var(--color-secondary-rgb),0.08)]"
                @click="handleLogout">
