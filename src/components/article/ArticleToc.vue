<template>
  <div v-if="tocItems.length > 0"
    ...
    <div class="flex items-center gap-2.5 mb-3 pb-2 border-b border-t-border">
      <List class="w-5 h-5 text-cyber-primary [filter:drop-shadow(0_0_3px_rgba(var(--color-primary-rgb),0.3))]" />
    ...
    <nav
      ...
      <a v-for="item in tocItems" :key="item.id" :href="'#' + item.id"
        class="block py-2 px-3 rounded-md text-sm text-cyber-body no-underline transition-all duration-200 relative hover:text-cyber-primary hover:bg-[rgba(var(--color-primary-rgb),0.06)] hover:translate-x-1 group/toc"
        :class="{
          'pl-4': item.level === 2,
          'pl-7 text-xs': item.level === 3,
        }" @click.prevent="scrollToHeading(item.id)">
        <span
          class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-0 bg-gradient-to-b from-cyber-primary to-cyber-pink rounded-full transition-[height] duration-250 [box-shadow:0_0_6px_rgba(var(--color-primary-rgb),0.4)] group-hover/toc:h-3/5"></span>
