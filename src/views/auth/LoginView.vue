  ...
  <div class="min-h-screen flex items-center justify-center bg-t-bg p-4 relative overflow-hidden">
    ...
    <div
      ...
      <div class="text-center mb-8">
        ...
        <p class="text-sm text-t-muted">欢迎回来</p>
      </div>

      <div class="flex mb-6 rounded-lg overflow-hidden border border-t-border">
        <button class="flex-1 py-2.5 text-sm font-semibold transition-all duration-300" :class="activeTab === 'password'
          ? 'bg-[rgba(var(--color-primary-rgb),0.12)] text-t-primary border-b-2 border-t-primary'
          : 'text-t-muted hover:text-t-body'" @click="activeTab = 'password'">密码登录</button>
        <button class="flex-1 py-2.5 text-sm font-semibold transition-all duration-300" :class="activeTab === 'email'
          ? 'bg-[rgba(var(--color-primary-rgb),0.12)] text-t-primary border-b-2 border-t-primary'
          : 'text-t-muted hover:text-t-body'" @click="activeTab = 'email'">邮箱登录</button>
      </div>
      ...
      <div class="text-center mt-6 text-sm text-t-muted">
        还没有账号？
        <router-link to="/register" class="text-t-primary font-medium hover:underline"
          style="text-shadow:0 0 6px rgba(0,240,255,0.3)">立即注册</router-link>
      </div>
    </div>
  </div>
</template>
