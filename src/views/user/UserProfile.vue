<template>
  <AppLayout :show-hero="false">
    <div class="max-w-4xl mx-auto">
      <div class="rounded-xl bg-[#111118] border border-[rgba(0,240,255,0.15)] overflow-hidden mb-6">
        ...
        <div class="px-6 pb-5 -mt-10 relative z-10">
          <div class="flex items-end justify-between">
            <div class="flex items-end gap-4">
              <div class="w-20 h-20 rounded-full bg-[#111118] border-2 border-[#00f0ff] flex items-center justify-center overflow-hidden shadow-[0_0_16px_rgba(17,17,24,0.9)]">
                <img :src="resolveAvatar(targetUser?.avatar)" alt="" class="w-full h-full object-cover" @error="handleAvatarError" />
              </div>
              <div class="pb-1">
                <h1 class="text-xl font-bold text-white">{{ targetUser?.nickname || '用户' }}</h1>
                <p class="text-sm text-[#6b7280]">@{{ targetUser?.username }}</p>
              </div>
            </div>
            <div v-if="isSelf" class="flex items-center gap-2 pb-1">
              <button
                class="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-medium bg-[rgba(0,240,255,0.1)] border border-[rgba(0,240,255,0.2)] text-[#00f0ff] transition-all duration-300 hover:bg-[rgba(0,240,255,0.18)] hover:shadow-[0_0_8px_rgba(0,240,255,0.12)]"
                @click="router.push('/profile/edit')">
                <PenLine class="w-3.5 h-3.5" />
                编辑资料
              </button>
              <button
                class="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-medium bg-[rgba(255,45,120,0.08)] border border-[rgba(255,45,120,0.2)] text-[#ff2d78] transition-all duration-300 hover:bg-[rgba(255,45,120,0.15)] hover:shadow-[0_0_8px_rgba(255,45,120,0.12)]"
      ...
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-lg font-semibold text-white">{{ isSelf ? '我的文章' : '已发布文章' }}</h2>
        </div>
        <button v-if="isSelf"
      ...
      <div v-else-if="articles.length > 0" class="flex flex-col gap-3">
        ...
        <Pagination v-if="total > pageSize" :current="currentPage" :total="total"
