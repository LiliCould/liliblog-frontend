  ...
  <div class="space-y-6">
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-t-border">
              ...
              <th class="text-left px-5 py-3.5 text-t-muted font-medium">文章数</th>
          ...
          <tbody>
            <tr v-for="tag in tags" :key="tag.id"
              ...
              <td class="px-5 py-3.5">
                <div class="flex items-center gap-2">
                  <span class="w-4 h-4 rounded-full border border-t-border flex-shrink-0" :style="{ backgroundColor: tag.color }"></span>
                  <span class="text-t-body text-xs font-mono">{{ tag.color }}</span>
                </div>
              </td>
              <td class="px-5 py-3.5 text-t-muted">{{ tag.articleCount }}</td>
              <td class="px-5 py-3.5 text-t-muted">{{ formatDate(tag.createTime) }}</td>
              <td class="px-5 py-3.5">
                <div class="flex items-center justify-end gap-1">
                  <button
                    class="p-1.5 rounded-lg text-t-muted transition-all duration-200 hover:text-t-primary hover:bg-[rgba(var(--color-primary-rgb),0.08)] cursor-pointer"
    ...
    <Teleport to="body">
      <div v-if="showDialog" class="fixed inset-0 z-[1200] flex items-center justify-center">
        ...
        <div class="relative w-full max-w-md mx-4 rounded-xl p-6 border border-t-border bg-t-surface">
          ...
          <div class="space-y-4">
            ...
            <div>
              <label class="block text-xs text-t-muted mb-1">颜色 <span class="text-[#f43f5e]">*</span></label>
              <div class="flex items-center gap-3">
                <input v-model="form.color" type="color"
                  class="w-10 h-10 rounded-lg border border-t-border cursor-pointer p-0.5 bg-t-bg" />
                <input v-model="form.color" type="text"
                  class="flex-1 px-3 py-2 rounded-lg bg-t-bg border border-t-border text-t-body text-sm font-mono outline-none transition-colors duration-200 focus:border-t-primary"
                  placeholder="#000000" />
              </div>
            </div>
          </div>
