                ...
                <td class="px-5 py-3.5">
                  <div class="flex items-center gap-2">
                    <span class="w-3 h-3 rounded-full flex-shrink-0"
                      :style="{ backgroundColor: tag.color || '#00f0ff', boxShadow: `0 0 6px ${tag.color || '#00f0ff'}40` }"></span>
                    <span class="text-t-body font-medium">{{ tag.name }}</span>
                  </div>
                </td>
                <td class="px-5 py-3.5">
                  <span class="text-t-muted font-mono text-xs">{{ tag.color || '-' }}</span>
                </td>
        ...
        <Transition enter-active-class="transition duration-200" enter-from-class="opacity-0"
          ...
          <div v-if="modalVisible" class="fixed inset-0 z-50 bg-[rgba(0,0,0,0.6)] flex items-center justify-center p-4"
            ...
            <div
              ...
              <h3 class="text-lg font-semibold text-t-title mb-5">{{ editingId ? '编辑标签' : '新增标签' }}</h3>
              ...
              <div class="space-y-4">
                ...
                <div>
                  <label class="block text-sm text-t-body mb-1.5">颜色</label>
                  <div class="flex items-center gap-3">
                    <input v-model="form.color" type="color"
                      class="w-10 h-10 rounded-lg border border-t-border bg-t-bg cursor-pointer" />
                    <input v-model="form.color" type="text"
                      class="flex-1 px-4 py-2.5 rounded-lg bg-t-bg border border-t-border text-t-body text-sm outline-none transition-all duration-300 focus:border-t-primary focus:shadow-[0_0_8px_rgba(var(--color-primary-rgb),0.15)] font-mono" />
                    <Palette class="w-5 h-5 text-t-muted flex-shrink-0" />
                  </div>
                </div>
              </div>
