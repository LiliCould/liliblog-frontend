  ...
  <AdminLayout>
    <div class="space-y-6">
      ...
      <div class="rounded-xl bg-[#111118] border border-[rgba(0,240,255,0.15)] overflow-hidden">
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-[rgba(0,240,255,0.15)]">
                ...
                <th class="text-left px-5 py-3.5 text-[#6b7280] font-medium">Slug</th>
                <th class="text-left px-5 py-3.5 text-[#6b7280] font-medium">描述</th>
                <th class="text-left px-5 py-3.5 text-[#6b7280] font-medium">排序</th>
            ...
            <tbody>
              ...
              >
                <td class="px-5 py-3.5 text-[#e0e0e8] font-medium">{{ cat.name }}</td>
                <td class="px-5 py-3.5 text-[#6b7280]">{{ cat.slug }}</td>
                <td class="px-5 py-3.5 text-[#6b7280] max-w-[200px] truncate">{{ cat.description || '-' }}</td>
                <td class="px-5 py-3.5 text-[#6b7280]">{{ cat.sortOrder }}</td>
                ...
                <td class="px-5 py-3.5">
                  <div class="flex items-center justify-end gap-1">
                    ...
                    >
                      <ToggleRight v-if="cat.status === 1" class="w-4 h-4 text-[#00f0ff]" />
        ...
        <div v-if="categories.length === 0 && !loading" class="text-center py-12 text-[#6b7280] text-sm">
          暂无分类
        </div>
      </div>
        ...
        >
          <div v-if="modalVisible" class="fixed inset-0 z-50 bg-[rgba(0,0,0,0.6)] flex items-center justify-center p-4" @click.self="closeModal">
            <div class="w-full max-w-md rounded-xl bg-[#111118] border border-[rgba(0,240,255,0.15)] p-6 shadow-[0_0_30px_rgba(0,240,255,0.1)]">
              <h3 class="text-lg font-semibold text-white mb-5">{{ editingId ? '编辑分类' : '新增分类' }}</h3>
              ...
              <div class="space-y-4">
                ...
                <div>
                  <label class="block text-sm text-[#e0e0e8] mb-1.5">Slug</label>
                  <input v-model="form.slug" type="text" class="w-full px-4 py-2.5 rounded-lg bg-[#0a0a0f] border border-[rgba(0,240,255,0.15)] text-[#e0e0e8] text-sm outline-none transition-all duration-300 focus:border-[#00f0ff] focus:shadow-[0_0_8px_rgba(0,240,255,0.15)]" />
                </div>
                <div>
                  <label class="block text-sm text-[#e0e0e8] mb-1.5">描述</label>
                  <textarea v-model="form.description" rows="3" class="w-full px-4 py-2.5 rounded-lg bg-[#0a0a0f] border border-[rgba(0,240,255,0.15)] text-[#e0e0e8] text-sm outline-none transition-all duration-300 focus:border-[#00f0ff] focus:shadow-[0_0_8px_rgba(0,240,255,0.15)] resize-none"></textarea>
                </div>
                <div>
                  <label class="block text-sm text-[#e0e0e8] mb-1.5">排序</label>
                  <input v-model.number="form.sortOrder" type="number" class="w-full px-4 py-2.5 rounded-lg bg-[#0a0a0f] border border-[rgba(0,240,255,0.15)] text-[#e0e0e8] text-sm outline-none transition-all duration-300 focus:border-[#00f0ff] focus:shadow-[0_0_8px_rgba(0,240,255,0.15)]" />
                </div>
                <div>
                  <label class="block text-sm text-[#e0e0e8] mb-1.5">状态</label>
                  <select v-model="form.status" class="w-full px-4 py-2.5 rounded-lg bg-[#0a0a0f] border border-[rgba(0,240,255,0.15)] text-[#e0e0e8] text-sm outline-none transition-all duration-300 focus:border-[#00f0ff] cursor-pointer">
