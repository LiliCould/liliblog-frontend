  ...
  <div class="space-y-6">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div
          ...
          <div class="flex items-center justify-between mb-3">
            ...
            <div class="w-9 h-9 rounded-lg bg-[rgba(var(--color-primary-rgb),0.1)] flex items-center justify-center">
        ...
        <div
          ...
          <div class="flex items-center justify-between mb-3">
            ...
            <div class="w-9 h-9 rounded-lg bg-[rgba(var(--color-secondary-rgb),0.1)] flex items-center justify-center">
      ...
      <div class="rounded-xl bg-t-surface border border-t-border p-6">
        ...
        <div v-if="loadingArticles" class="space-y-3">
          <div v-for="i in 5" :key="i" class="h-12 rounded-lg bg-t-elevated animate-pulse"></div>
        </div>

        <div v-else-if="recentArticles.length > 0" class="space-y-2">
          <div v-for="article in recentArticles" :key="article.id"
            class="flex items-center justify-between px-4 py-3 rounded-lg bg-t-bg border border-[rgba(var(--color-primary-rgb),0.08)] transition-all duration-300 hover:border-[rgba(var(--color-primary-rgb),0.3)]">
            ...
            <span class="text-xs text-t-muted ml-4 flex-shrink-0">{{ formatRelativeTime(article.createTime) }}</span>
          </div>
        </div>
